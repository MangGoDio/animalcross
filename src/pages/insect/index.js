...
export default function() {
  ...
  const [type, setType] = useState('map'),
    ...
    [model_list, setModelList] = useState(getMarkInfo('mango_insect_model') || []); // 标记列表
  ...
  const saveModelInfo = id => {
    ...
    setModelList(arr);
    setMarkInfo(arr, 'mango_insect_model');
  };
  ...
  return (
    <div style={{ padding: 8 }}>
      ...
      {type === 'list' && <InsectList {...props} />}
      {type === 'map' && <InsectMap {...props} />}
    </div>
  );
}
