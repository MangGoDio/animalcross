...
import SeafoodMap from './SeafoodMap';
...
export default function() {
  ...
  const [type, setType] = useState('map'),
    ...
    [mark_list, setMarkList] = useState(getMarkInfo('mango_seafood') || []); // 标记列表
  // [model_list, setModelList] = useState(getMarkInfo('mango_seafood_model') || []); // 标记列表
  ...
  //   }
  //   setModelList(arr);
  //   setMarkInfo(arr, 'mango_seafood_model');
  // };
  ...
  return (
    <div style={{ padding: 8 }}>
      ...
      {type === 'list' && <SeafoodList {...props} />}
      {type === 'map' && <SeafoodMap {...props} />}
    </div>
  );
}
